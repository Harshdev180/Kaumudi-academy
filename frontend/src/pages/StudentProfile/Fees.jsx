...
import { 
  CreditCard, Download, Receipt, History, 
  Wallet, CheckCircle2, AlertCircle, Coins, 
  ArrowUpRight, ShieldCheck, Landmark
} from 'lucide-react';
...
const FeePurchase = () => {
  ...
  const paymentHistory = [
    { date: 'July 1, 2024', desc: 'Tuition Fee Installment 1', amount: '15,000', status: 'Paid', type: 'Academic' },
    { date: 'August 5, 2024', desc: 'Exam Fee 2024', amount: '5,000', status: 'Paid', type: 'Examination' },
    { date: 'September 10, 2024', desc: 'Library Fee', amount: '1,000', status: 'Paid', type: 'Facility' },
  ];
    ...
    <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in duration-700 pb-12">
        ...
        {feeSummary.map((item, idx) => (
          ...
          >
            <div className="space-y-1">
              ...
              <h4 className={`text-3xl font-bold font-serif ${item.textColor}`}>₹ {item.amount}</h4>
            </div>
            <div className={`p-4 bg-white rounded-2xl shadow-sm ${item.textColor}`}>
              {item.icon}
            </div>
          </div>
        ))}
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-black/5 overflow-hidden">
        ...
        <div className="px-10 py-8 border-b border-gray-100 flex justify-between items-center bg-gray-50/30">
          <div className="flex items-center gap-3">
            ...
            <h4 className="text-sm font-black uppercase tracking-widest text-gray-700">Payment History</h4>
          </div>
                ...
                <tr key={idx} className="group hover:bg-[#fdfbf7] transition-colors">
                  ...
                  <td className="px-6 py-6">
                    <div className="space-y-1">
                      ...
                      <span className="text-[9px] font-black uppercase tracking-widest text-[#c9a050]/70">{item.type}</span>
                    </div>
                  </td>
                  <td className="px-6 py-6">
                    <p className="text-base font-bold text-[#74271E]">₹ {item.amount}</p>
                  </td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-gradient-to-br from-[#74271E] to-[#4a1813] p-10 rounded-[2.5rem] text-white flex items-center gap-8 relative overflow-hidden">
           <Landmark className="absolute -right-6 -bottom-6 opacity-10" size={180} />
           ...
           <div className="relative z-10">
             <h5 className="font-serif font-bold text-xl mb-2">Secure Transactions</h5>
             <p className="text-sm text-white/60 leading-relaxed font-light">
               All payments are encrypted using bank-grade security protocols. We accept all major cards, UPI, and Net Banking.
             </p>
           </div>
        </div>
        ...
        <div className="bg-[#fdfbf7] p-10 rounded-[2.5rem] border border-[#e6d5b8]/30 flex flex-col justify-center">
          <div className="flex items-center gap-4 mb-4">
            ...
            <p className="text-[11px] font-black uppercase tracking-widest text-gray-700">Need Assistance?</p>
          </div>
          <p className="text-sm text-gray-500 mb-6">
            If you encounter any issues regarding your fee installments or receipt generation, please contact the Registrar's Office.
          </p>
          <div className="flex gap-4">
            <button className="text-[10px] font-black uppercase tracking-[0.15em] text-[#74271E] border-b-2 border-[#74271E] pb-1">Raise a Ticket</button>
            <button className="text-[10px] font-black uppercase tracking-[0.15em] text-gray-400 hover:text-gray-600 pb-1">Billing FAQ</button>
          </div>
        </div>
      </div>
    </div>
