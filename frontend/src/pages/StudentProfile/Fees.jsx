...
const FeePurchase = () => {
  ...
  const loadRazorpay = () => {
    return new Promise((resolve, reject) => {
      ...
      console.log("Checking Razorpay: window.Razorpay =", typeof window.Razorpay);
      ...
      script.onload = () => {
        console.log("Razorpay loaded, window.Razorpay:", typeof window.Razorpay);
        if (window.Razorpay && typeof window.Razorpay === 'function') {
          resolve();
        } else {
    ...
    try {
      ...
      if (paymentMode === "EMI") {
        ...
        if (!orderResponse.success) {
          alert(orderResponse.message || "Failed to create EMI installment order");
          return;
        }
      } else {
