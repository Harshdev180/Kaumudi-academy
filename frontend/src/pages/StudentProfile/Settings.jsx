...
import { 
  ...
  Shield, Users, MapPin, Clock, Mail, Smartphone, Monitor, X
} from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-700 pb-12">
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col md:flex-row items-center gap-8 p-6">
          ...
          <div className="text-center md:text-left space-y-2">
            <h2 className="text-4xl font-serif font-bold text-gray-800 tracking-tight">
              {profile.firstName} {profile.lastName} <span className="text-gray-300 mx-2 font-light">/</span> {profile.hindiName}
            </h2>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-black/5 overflow-hidden min-h-[550px]">
        ...
        <div className="p-12 relative">
          ...
          {activeTab === 'notification settings' && <NotificationView notifications={notifications} setNotifications={setNotifications} />}
          ...
          <form className="space-y-6">
            {['Current Password', 'New Password', 'Confirm Password'].map((label) => (
              <div key={label}>
                ...
                <input type="password" className="w-full px-6 py-4 bg-white border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-[#74271E]/10 text-sm font-medium" placeholder="••••••••" />
              </div>
            ))}
            <button type="button" className="w-full mt-4 bg-[#74271E] text-white py-4 rounded-2xl font-bold text-sm shadow-xl hover:bg-[#5a1e17] transition-all flex items-center justify-center gap-2">
...
const NotificationView = ({ notifications, setNotifications }) => (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        {[
          ...
          <div key={pref.id} className="flex items-center justify-between p-6 rounded-[2rem] bg-gray-50/50 hover:bg-white border border-transparent hover:border-gray-100 transition-all group">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-gray-400 group-hover:text-[#c9a050] shadow-sm">{pref.icon}</div>
              <div>
      ...
      <div className="bg-[#fdfbf7] p-10 rounded-[3rem] border border-[#e6d5b8]/30 flex flex-col justify-center text-center space-y-6">
        <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto text-[#c9a050] shadow-sm"><ShieldCheck size={32} /></div>
        <div>
          <h4 className="text-sm font-black uppercase tracking-widest text-gray-700 mb-2">Privacy Assurance</h4>
          <p className="text-xs text-gray-500 leading-relaxed font-medium italic">"We respect your peace. Notifications are sent only for essential academic progress."</p>
        </div>
      </div>
    </div>
  </div>
